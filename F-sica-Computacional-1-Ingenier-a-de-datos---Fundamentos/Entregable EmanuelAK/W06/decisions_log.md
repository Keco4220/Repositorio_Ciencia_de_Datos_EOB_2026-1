...
## Decision 9 — Usar surrogate key (host_id) en dim_host_sk (W06)

- **Fecha:** 2025-01-24
- **Decision:** Reemplazar el JOIN por hostname (VARCHAR) por un JOIN por host_id (INTEGER surrogate).
- **Razon:** 1) host_id ocupa 4 bytes vs ~15 bytes de hostname promedio — JOINs mas eficientes con datos grandes. 2) Si NASA renombra una estrella, solo hay que actualizar dim_host_sk.hostname sin tocar fact_planet_sk. 3) El PRIMARY KEY constraint garantiza unicidad formalmente, no solo por convencion.
- **Evidencia:** dim_host_sk: n_rows=4537, n_keys=4537. fact_planet_sk: n_fact_sk=6081, orphan_rows=0. FK completamente satisfecha.
- **Alternativas:** Mantener JOIN por hostname (mas simple, ya funcionaba) -> rechazada porque no escala y no tiene garantia formal.

## Decision 10 — Metricas elegidas para Gold (W06)

- **Fecha:** 2025-01-24
- **Decision:** Incluir avg_pl_rade, avg_pl_bmasse, min/max_disc_year en gold_by_discoverymethod, y avg_pl_rade + avg_pl_bmasse en gold_by_host.
- **Razon:** Son las preguntas mas frecuentes en analisis de exoplanetas: tamano tipico y masa tipica de los planetas detectados por cada metodo, y rango historico de descubrimientos. Permiten comparar los sesgos de cada tecnica de deteccion directamente.
- **Evidencia:** Transit avg_rade=4.4 R🜨 vs Imaging avg_rade=13.4 R🜨 — diferencia de 3x que refleja el sesgo de deteccion de cada metodo. TRAPPIST-1 avg_bmasse=0.9 M🜨 vs HD 10180 avg_bmasse=1587 M🜨 — ilustra la diversidad de sistemas multiplanetarios.
